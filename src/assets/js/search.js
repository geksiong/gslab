(async function () {
  ...
  searchBox.addEventListener("keyup", function (e) {
    ...
    timeout = setTimeout(() => {
      ...
      history.replaceState(
        null,
        null,
        `?q=${this.value}&start=${start}&size=${size}`
      );
      display();
    }, 500);
  });
  ...
  function display() {
    wrapper.innerHTML = "";
    if (query === null || query === "")
      return (info.textContent = "Enter keywords in the search box above");
    let filteredPosts = posts.filter((post) => {
      let postTitle = post.title.toLowerCase();
      return postTitle.indexOf(query) !== -1;
    });
    if (filteredPosts.length === 0)
      return (info.textContent = `No results found for "${query}"`);
    ...
    info.textContent = `Showing ${show} of ${resultsFound} result${
      resultsFound > 1 ? "s" : ""
    } found`;
